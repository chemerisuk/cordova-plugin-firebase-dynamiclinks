...
+ (void)load {
    ...
    dispatch_once(&onceToken, ^{
        [self FirebaseDynamicLinksPlugin_swizzleMethod:@selector(application:openURL:options:)];
        [self FirebaseDynamicLinksPlugin_swizzleMethod:@selector(application:continueUserActivity:restorationHandler:)];
    });
}

+ (void)FirebaseDynamicLinksPlugin_swizzleMethod:(SEL)originalSelector {
    Class class = [self class];
    NSString *selectorString = NSStringFromSelector(originalSelector);
    SEL newSelector = NSSelectorFromString([@"FirebaseDynamicLinksPlugin_" stringByAppendingString:selectorString]);
...
- (BOOL)FirebaseDynamicLinksPlugin_application:(UIApplication *)app openURL:(NSURL *)url options:(NSDictionary<NSString *, id> *)options {
    // always call original method implementation first
    BOOL handled = [self FirebaseDynamicLinksPlugin_application:app openURL:url options:options];
...
- (BOOL)FirebaseDynamicLinksPlugin_application:(UIApplication *)app continueUserActivity:(NSUserActivity *)userActivity restorationHandler:(void (^)(NSArray *))restorationHandler {
    // always call original method implementation first
    BOOL handled = [self FirebaseDynamicLinksPlugin_application:app continueUserActivity:userActivity restorationHandler:restorationHandler];
    FirebaseDynamicLinksPlugin* dl = [self.viewController getCommandInstance:@"FirebaseDynamicLinks"];
